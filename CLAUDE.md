...
- **Site web : thème clair unique, sobre (« keep it simple »).** Fond `#FAFAF7`, texte `#111418`, un seul accent bleu marine `#14335C`, filets `#DDE1E6`, Georgia + police système. Pas de mode sombre, pas de bouton de lumière. Les visuels (image de partage, vidéo d’introduction, favicon) restent sur fond encre `#0B1A28` : ce sont des images, pas le thème.
- **Animations autorisées sur le site (peu, mais fortes)** : fondu entre les pages (`@view-transition`), photo du hero qui se pose, nom mot par mot, filet qui se trace, compteurs, boutons du hero en décalé, cascade des lignes de liste (`.cascade`, une fois), remplissage des boutons au survol, soulignement de la navigation, léger relief des vidéos au survol, flash des résultats de démo. Rien d’autre ; tout coupé par `prefers-reduced-motion`.
